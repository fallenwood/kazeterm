...
fn build_theme(colors: &AlacrittyColors) -> Option<ThemeFile> {
    ...
    let theme_colors = ThemeColors {
        background: colors.primary.background.as_ref().map(|s| normalize_color(s)),
        foreground: colors.primary.foreground.as_ref().map(|s| normalize_color(s)),
        accent: colors
            .primary
            .foreground
            .as_ref()
...
"##;
        ...
        assert_eq!(result.config_patch.font_family.as_deref(), Some("JetBrains Mono"));
