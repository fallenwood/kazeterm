...
- `defaults.rs` constructs the default `SettingsStore`; `init_gpui_kit_theme` mirrors Kazeterm colors and fonts into `gpui_kit::component::Theme`.

## Flow

1. The application builds a `SettingsStore` from `config::Palette` and installs it as a GPUI global.
2. UI components read the store through `ActiveTheme`; GPUI Kit controls receive equivalent colors and fonts through `init_gpui_kit_theme`.
...
- Depends on: `config` for palettes and font configuration; `gpui` and `gpui-kit` for global state and widget styling.
